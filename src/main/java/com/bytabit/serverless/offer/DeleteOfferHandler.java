...
@Slf4j
public class DeleteOfferHandler extends WebRequestHandler {
    ...
    public ApiGatewayResponse handleRequest(Map<String, Object> input, Context context) {
        try {
            ...
            Offer deletedOffer = offerManager.delete(id);

            // send the response back
            return ApiGatewayResponse.builder()
                    .setStatusCode(200)
                    .setObjectBody(deletedOffer)
                    //.setHeaders(Collections.singletonMap("X-Powered-By", "AWS Lambda & Serverless"))
                    .build();

        } catch (Exception ex) {
            log.error("Error in delete offer: {}\n{}", ex, ex.getStackTrace());

            // send the error response back
            Response responseBody = new Response("Error in deleting offer: ", input);
            return ApiGatewayResponse.builder()
                    .setStatusCode(500)
